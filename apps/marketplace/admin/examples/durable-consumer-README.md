...
- **Idempotent retry:** repeat one `action` command with the same operation name and exact raw JSON, preferably immediately. The SQLite operations table persists method, path, body and retry_key **before** sending. The API returns its cached result and `Idempotent-Replayed: true`. A lost HTTP response/restart uses the same stored key. Changed inputs with the same operation name are rejected locally. Keys are scoped to credentials. Completed responses remain stored until the simulator data is cleared; after credential rotation or a data reset inspect current state rather than blindly creating another shipment. Use the console’s credential revocation preview to plan API-client and Tokopedia receiver key changes before revoking.
