...
export function DetailPanel({ detail, token, onClose, onRefresh, onNotice }: DetailPanelProps) {
  ...
  const [newInventory, setNewInventory] = useState<{ product_id: string; on_hand_quantity: number | string }>({ product_id: "", on_hand_quantity: 0 });
  const detailPath =
    detail.type === "order"
  ...
  useEffect(() => {
    request<DetailData>(detailPath, token)
      .then(setData)
      .catch((err: Error) => setError(err.message));
  }, [detailPath, token]);
  useEffect(() => {
    if (detail.type !== "warehouse" || !data?.shop_id) return;
    request<ListResponse<ProductSummary>>(`/control/v1/shops/${data.shop_id}/products?limit=100`, token)
  ...
  const saveInventory = async (productID: string, onHandQuantity: unknown) => {
    ...
    setError("");
    try {
      await request<unknown>(`/control/v1/warehouses/${detail.id}/inventory/${productID}`, token, {
        method: "PUT",
        body: JSON.stringify({ on_hand_quantity: quantity }),
      });
      setData(await request<DetailData>(detailPath, token));
  ...
  const transition = async (action: string) => {
    await request<unknown>(`/control/v1/orders/${detail.id}/actions/${action}`, token, {
      method: "POST",
      body: JSON.stringify({}),
    });
    setData(await request<DetailData>(`/control/v1/orders/${detail.id}`, token));
    await onRefresh();
  ...
  const shipmentTransition = async (action: string) => {
    ...
    try {
      ...
      await request<unknown>(`/control/v1/shipments/${shipmentID}/actions/${action}`, token, {
        method: "POST",
  ...
  const replay = async (id: string) => {
    await request<unknown>(`/control/v1/events/${id}/replay`, token, { method: "POST" });
    setData(await request<DetailData>(`/control/v1/orders/${detail.id}`, token));
    await onRefresh();
    onNotice("Event replay queued");
  };
  const duplicate = async (id: string) => {
    await request<unknown>(`/control/v1/events/${id}/duplicate`, token, {
      method: "POST",
    });
    setData(await request<DetailData>(`/control/v1/orders/${detail.id}`, token));
    await onRefresh();
  ...
  const delay = async (id: string) => {
    ...
    await request<unknown>(`/control/v1/events/${id}/delay`, token, {
      method: "POST",
