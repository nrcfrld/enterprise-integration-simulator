...
interface DeveloperPortalProps {
  ...
  onDestination?: (destination: PortalDestination) => void;
  controlToken?: string;
  shop?: Shop;
...
function TryIt({ resourceID, packageID, controlToken, drafts, shop, knownCredential, api, activeEndpointID, onSelect, credentials, onCredentialsChange, onNavigate }: {
  resourceID?: string;
  packageID?: string;
  controlToken?: string;
  ...
  onSelect: (endpointID: string, resourceID?: string, packageID?: string) => void;
...
export function DeveloperPortal({ destination, onDestination, controlToken, shop, api, onNavigate, credentialHandoff, onHandoffConsumed }: DeveloperPortalProps) {
  ...
  const selectEndpoint = (id: string, resourceID?: string, packageID?: string) => {
    setLocalEndpointID(id); setLocalSection("try");
    onDestination?.({ section: "try", endpoint: id, resourceID, packageID });
  };
  ...
  useEffect(() => {
    const revoked = (event: Event) => {
      if ((event as CustomEvent<{ clientID: string }>).detail.clientID !== credentials.clientID) return;
      setCredentials({ clientID: "", secret: "", accessToken: "" });
      setKnownCredential(undefined); drafts.current.clear(); setDraftGeneration(value => value + 1);
    };
    window.addEventListener("marketplace:credential-revoked", revoked);
    return () => window.removeEventListener("marketplace:credential-revoked", revoked);
  }, [credentials.clientID]);
  const openTry = (endpointID: string) => selectEndpoint(endpointID);
