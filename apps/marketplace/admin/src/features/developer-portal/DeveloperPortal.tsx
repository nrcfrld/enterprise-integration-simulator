...
function TryIt({ api, activeEndpointID, onSelect, credentials, onCredentialsChange, onNavigate }: {
  ...
  return <><section className="try-heading"><h2>Make a signed request to the provider you are integrating with.</h2><p>Choose a provider contract first. The simulator signs and sends the exact request shape used by the selected Marketplace API.</p></section><div className="contract-switcher join" aria-label="Provider contract">{contracts.map((contract) => <button key={contract} type="button" className={`btn btn-sm join-item ${endpoint.contract === contract ? "selected btn-primary" : "btn-ghost"}`} onClick={() => onSelect(ENDPOINTS.find((item) => item.contract === contract)?.id ?? endpoint.id)}>{contract === "shared" ? "Shared resources" : contract === "shopee" ? "Shopee-like" : "Tokopedia-like"}</button>)}</div><CredentialPanel credentials={credentials} onChange={onCredentialsChange} onClear={() => onCredentialsChange({ clientID: "", secret: "", accessToken: "" })} onNavigate={onNavigate} contract={endpoint.contract} /><section className="operation-picker card bg-base-100"><div><h3>Choose an operation</h3><p>Start with a list or search request. It returns ids you can paste into the next lifecycle operation.</p></div><div>{ENDPOINTS.filter((item) => item.contract === endpoint.contract).map((item) => <button type="button" key={item.id} className={`btn ${item.id === endpoint.id ? "selected btn-primary" : "btn-ghost"}`} onClick={() => onSelect(item.id)}><HttpMethod method={item.method} /><span><b>{item.title}</b><small>{item.group}</small></span></button>)}</div></section><RequestSimulator key={endpoint.id} endpoint={endpoint} api={api} credentials={credentials} /><CodeExamples endpoint={endpoint} /></>;
}
