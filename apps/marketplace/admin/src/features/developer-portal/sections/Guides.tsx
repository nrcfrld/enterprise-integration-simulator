...
export function Authentication() {
  return <><section className="reference-heading"><h2>Choose the signature that matches the path</h2><p>A dashboard login never authenticates a public integration request. All three integration contracts use the credential secret, but their signing inputs differ.</p></section><section className="explanation-flow"><article><b>Shared resources</b><p>For <code>/api/v1</code>, send X-Client-Id, X-Timestamp, and X-Signature. Sign <code>METHOD + PATH + TIMESTAMP + raw body</code>.</p></article><article><b>Shopee-like</b><p>For <code>/api/shopee/v1</code>, send X-Shopee-Partner-Id, X-Shopee-Timestamp, and X-Shopee-Signature. Sign <code>PARTNER_ID + PATH + TIMESTAMP + raw body</code>; the HTTP method is not included.</p></article><article><b>Tokopedia-like</b><p>For <code>/api/tokopedia/v202309</code>, send app_key, timestamp, sign, and x-tts-access-token. Sort query keys before signing; exclude sign and access_token.</p></article></section><section className="reference-callout"><b>HMAC signature</b><p>An HMAC is a tamper-check made with your secret. Never send the secret itself. Requests older than five minutes are rejected, so generate a new timestamp for every attempt.</p></section></>;
}

export function ControlPlaneAccounts({ api }: { api: string }) {
  return <><section className="reference-heading"><h2>Manage Control Plane accounts</h2><p>Control Plane accounts open the Admin console. They are separate from integration credentials and never sign public API requests.</p></section><section className="reference-callout"><b>Account roles</b><p>Self-registration creates an Operator account. Admins can create other Admin users and review all Control Plane accounts from the Users workspace.</p></section><div className="portal-actions"><Link to={controlDestination("Users")}>Open Users</Link></div><ControlPlaneRegistrationSimulator api={api} /></>;
}
