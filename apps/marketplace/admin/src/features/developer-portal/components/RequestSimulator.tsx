  ...
  return (
    <section className="request-simulator">
      ...
      <form onSubmit={run}>
        ...
        <div className="simulator-actions"><button className="btn btn-primary" type="submit" disabled={state.status === "loading" || Boolean(blockedReason)}>{state.status === "loading" ? "Sending signed request…" : "Send signed request"}</button><button type="button" className="btn btn-ghost" onClick={reset}>Reset request</button>{lastAttempt && <button className="btn btn-outline" type="button" disabled={state.status === "loading" || Boolean(blockedReason)} onClick={() => { restore(lastAttempt); void send(lastAttempt, true); }}>Retry same operation</button>}
          {mutation && <button className="btn btn-ghost" type="button" disabled={state.status === "loading"} onClick={() => { setIdempotencyKey(createIdempotencyKey()); setLastAttempt(undefined); setLastClientID(undefined); setPrepared(undefined); setState(initialState); }}>New operation</button>}
          {state.status === "loading" && <button className="btn btn-error btn-soft" type="button" onClick={cancel}>Stop waiting</button>}
        </div>
      </form>
