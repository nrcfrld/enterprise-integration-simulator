...
it("shows active faults, refreshes recovery, and keeps failed status unknown", async () => {
  ...
  expect(screen.getByText(/Global maintenance is ON/)).toBeVisible();
  request.mockRejectedValue(new Error("Offline"));
  fireEvent.click(screen.getByRole("button", { name: "Refresh fault status" }));
  ...
  fireEvent.click(screen.getByRole("button", { name: "Refresh fault status" }));
  expect(await screen.findByText("No shop faults enabled at last check.")).toBeVisible();
});
