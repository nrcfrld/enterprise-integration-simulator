...
export function WebhookVerification({ provider, signingClientID }: { provider?: ProviderProfile; signingClientID?: string }) {
  return <section className="reference-callout webhook-verification" aria-label="Webhook verification contract">
    <header className="webhook-verification-intro">
    ...
    <div className="webhook-verification-contracts">
      {provider !== "TOKOPEDIA_LIKE" && <article>
    ...
    <p className="webhook-verification-note">Verify the exact raw bytes before parsing or processing; reject timestamps outside your freshness window. Payload data uses canonical simulator fields and statuses. After verification, durably deduplicate the event and fetch current state through the provider API; events can arrive late or out of order.</p>
  </section>;
}
